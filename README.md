...
- 公开入口为 `/knowledge/`，游客可浏览视频、图文和图片课程，也可查看公开问答；登录后可保存学习进度、收藏、发表评论或回复他人评论、提问、回答、点赞和采纳回答。
- 课程详情支持章节、提示框、清单和图文说明，并内置基于当前课程内容的 AI 学习问答。已提供完整示范课“棉花苗期14天田间诊断课”，覆盖查苗、缺苗判断、定苗和苗期异常巡查。
- 图文课程可配置课后选择题。用户提交后会获得即时判分和逐题解析，错题可调用 AI 按课程标准解析重新讲解；公开论坛用于用户发起棉花种植问题并由其他用户共同解答，不设置专家角色或专家专属权限。
- 管理员从 `/knowledge/admin.html` 上架视频、图片和文案，设置分类、课后小测、推荐位与发布状态；评论可按专区、课程、状态和关键词筛选。管理员登录后访问公开知识讲堂会自动进入浏览管理模式，可在课程卡片、课程详情和问答页直接编辑、上下架或处理内容，普通用户不会看到管理控件。
- 农户小程序首页“知识讲堂”入口通过 `web-view` 打开正式网页。已登录农户会通过 2 分钟有效、只能兑换一次的票据自动登录网页，长期 JWT 不会出现在 URL 中。真机上线前须在微信公众平台配置与 `utils/auth.js` 生产地址一致的 HTTPS 业务域名。
- 网页支持微信开放平台网站应用扫码登录。生产环境需配置 `WECHAT_WEB_APPID`、`WECHAT_WEB_SECRET` 和 `WECHAT_WEB_REDIRECT_URI`，并将网站应用与小程序绑定到同一微信开放平台账号；农户至少从已登录小程序进入一次知识讲堂后，服务端会补全 `unionid`，后续电脑扫码才能准确关联原账号。

#### 知识讲堂视频存储

- 后台“本机上传”当前保存到 `server/public/uploads/knowledge/`；Docker 部署时对应 `uploads` 持久卷，重建容器不会丢失，但文件会占用云服务器磁盘，播放流量也会消耗服务器带宽。
- 初期图片和少量短视频可以继续本机存储；正式课程视频推荐使用腾讯云 COS、阿里云 OSS 等对象存储，并绑定 CDN。上传后把公开 HTTPS 地址填入后台“视频地址”，现有播放器可直接使用。
- 对象存储建议启用分片上传、HTTP Range、CDN 缓存和防盗链，并保留原始视频；网页播放版本优先转为 H.264 + AAC 的 MP4，控制在 720p/1080p。
- 本机单文件上限由 `KNOWLEDGE_LOCAL_UPLOAD_MAX_MB` 控制，默认 250 MB。上线前应同时监控 Docker `uploads` 卷容量并制定备份或迁移策略。
...
node db/migrate_knowledge_hall.js      # 公共知识讲堂、学习记录、评论、收藏和论坛问答
