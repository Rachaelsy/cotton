...
const LOCAL_IP = '192.168.0.28'      // 改成你电脑的局域网 IP（ipconfig 查询）
```
...
Base URL（开发）：`http://192.168.0.28:3000`
...
订单状态流转：`pending_payment`（待付款，含30分钟超时）→ `pending_ship`（待发货）→ `shipped`（已发货）→ `completed`（已完成）/ `refund`（售后中）；超时/主动取消 → `cancelled`（库存自动释放）
