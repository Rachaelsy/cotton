...
function run() {
  ...
  const storePage = read('subpkg-supplies', 'supplies-store', 'index.js')
  const detailPage = read('subpkg-supplies', 'supplies-detail', 'index.js')
  ...
  assert.ok(checkout.includes('couponHint'), 'checkout should explain why a claimed coupon cannot be applied')
  assert.ok(checkout.includes('baseQuote.merchant_id || group.merchant_id'), 'coupon matching should use the server-authoritative merchant id')
  ...
  assert.ok(suppliesPage.includes('stock <= 0'), 'supplies list must block zero-stock products before adding them')
  assert.ok(storePage.includes('stock <= 0'), 'merchant store must block zero-stock products before adding them')
  assert.ok(detailPage.includes('stock <= 0'), 'product detail must block zero-stock purchase actions')
  assert.ok(read('subpkg-supplies', 'supplies-cart', 'index.wxml').includes('{{quoteError}}'), 'quote failures should be visible instead of silently showing the original price')
