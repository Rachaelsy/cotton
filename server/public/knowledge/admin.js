...
async function api(path, options={}) {
  ...
  const result=await response.json().catch(()=>({msg:'请求失败'})); if(response.status===401){location.href='/admin/login.html';throw new Error('登录已过期')} if(!response.ok||result.code!==200)throw new Error(result.msg||'请求失败'); return result.data
}
