...
assert(updateScript.includes('command -v node'))
assert(updateScript.includes('docker run --rm --network none'))
assert(updateScript.includes('docker.m.daocloud.io/library/node:20-alpine'))
assert(updateScript.includes('docker compose up -d --build --force-recreate --remove-orphans'))
