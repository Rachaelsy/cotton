...
喀什优棉公共服务平台提供独立、可扩展的小程序管理员账号，不需要把账号提升为核心平台管理员。后台“政策资讯”统一管理政策中心与行业资讯，支持 Markdown 编辑和分类发布；“优棉金融”维护种植贷、棉花保险、期货基础和金融政策解读四篇 Markdown 文章；“首页推送”集中管理政策、行业、金融和首页专稿，最多同时推送 5 篇；“语音播报”展示农户在小程序按需生成并自动保存的播报，支持人工修改和状态管理；“专家讲堂”管理精选问答与在线专家，并支持头像和问答封面上传；“农机服务”和“农资服务”用于维护小程序中展示的具体产品目录；“加工服务”用于维护加工厂、地图坐标、产能、联系人、图片与发布状态；“地块管理”用于维护按地块展示的今日农事。首次部署或更新后，容器启动会自动创建或升级管理员、文章、专家、播报和生产服务数据表。

政策资讯文章支持公开读取评论、登录农户发表评论；评论按文章和用户保存到 `policy_comments`，限制为 2–300 字，并对短时间重复提交进行拦截。小程序收藏保存在当前设备，转发使用微信原生分享能力。

优棉金融不接入或展示期货行情，仅提供种植贷、棉花保险、期货基础和金融政策的知识科普与官方入口指引。服务端不保留行情接口或行情供应商密钥配置，也不提供期货开户、交易撮合、荐单或收益承诺。
...
“专家讲堂”分为精选问答和在线专家。问答维护棉花生产常见问题与专家答案，专家维护公开简介和在线状态；头像与问答封面上传到 `community_uploads` 持久化卷。管理接口使用 `/api/expert-studio/admin/*`；小程序通过共享 `/api/expert` 读取，公共服务网页通过 `/api/expert-studio/public` 读取相同数据表，因此后台发布后两个终端同时生效。精选视频和 AI 陪练入口已移除，历史视频记录保留在数据库但不再通过公开或管理接口返回。部署该功能需要重新构建 `app`、`community` 和 `nginx`，以同步核心公开接口、后台和代理规则。
...
商业网站提供商品浏览、数量选择、收藏、购物车、在线下单、订单查询和售后入口，并与
`cotton-app` 共用商品、商户、用户和订单 API。商品成交价格、库存、配送费用与支付方式以
订单确认页为准；农资标签、登记信息、执行标准和批次由对应经营主体维护并承担履约责任。
农机服务按地块、设备档期、服务半径和作业参数确认后进入预约与履约流程。
...
仪表盘也提供公益、商业和内容运营入口。
优棉学堂视频由运营人员在腾讯云 VOD 控制台上传和转码，后台只保存系列课程、HTTPS MP4/M3U8 播放地址、封面地址和课程元数据，不提供视频上传入口。小程序使用原生 `video` 组件播放，并按有效观看片段记录进度；覆盖视频实际时长 95% 后自动完成课时并发放一次学习积分。
