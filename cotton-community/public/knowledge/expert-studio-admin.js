(() => {
  ...
  function render() {
    ...
    if (state.tab === 'experts') {
      ...
      body.innerHTML = items.length ? items.map(item => `<tr><td><strong>${esc(item.title)}</strong><br><span class="question-excerpt">${esc(item.intro || item.subtitle || item.content)}</span></td><td>${esc(item.expertName || '平台专家')}<br><span class="question-excerpt">${esc(item.categoryName)}</span></td><td>${item.coverUrl ? '含封面' : '纯文字'}</td><td>${item.isFeatured ? '是' : '否'}</td><td>${status(item.isPublished)}</td><td>${item.sortOrder}</td><td>${item.updatedAt ? new Date(item.updatedAt).toLocaleString('zh-CN') : '—'}</td><td><div class="product-actions"><button class="small-btn primary" data-studio-content-edit="${item.id}">编辑</button><button class="small-btn danger" data-studio-content-delete="${item.id}">删除</button></div></td></tr>`).join('') : '<tr><td colspan="8">暂无精选问答，点击右上角新增。</td></tr>'
    }
  ...
  function setTab(tab) {
    state.tab = ['qa', 'experts'].includes(tab) ? tab : 'qa'
    ...
    $('addExpertStudioBtn').textContent = state.tab === 'experts' ? '＋ 新增专家' : '＋ 新增问答'
    render()
  }
  ...
  function openContent(id = 0) {
    ...
    $('studioContentType').value = 'qa'; expertOptions(item ? item.expertId : '')
    $('studioContentId').value = item ? item.id : ''; $('expertContentModalTitle').textContent = item ? '编辑精选问答' : '新增精选问答'
    ...
    $('studioContentCover').value = item ? item.coverUrl : ''
    ...
    $('studioContentFeatured').checked = item ? item.isFeatured : true; $('studioContentPublished').checked = item ? item.isPublished : true; $('studioCoverUploadStatus').textContent = ''; $('expertContentMessage').textContent = ''; $('expertContentModal').classList.remove('hidden')
  }
  ...
  async function saveContent(event) {
    ...
    const body = { type: 'qa', title: $('studioContentTitle').value.trim(), subtitle: $('studioContentSubtitle').value.trim(), expert_id: $('studioContentExpert').value, category_key: category[0], category_name: category[1], cover_url: $('studioContentCover').value.trim(), intro: $('studioContentIntro').value.trim(), content: $('studioContentBody').value.trim(), tags: lines($('studioContentTags').value), sort_order: $('studioContentSort').value, is_featured: $('studioContentFeatured').checked, is_published: $('studioContentPublished').checked }
    if (!body.title || !body.content) { $('expertContentMessage').textContent = '问题标题和专家答案不能为空'; return }
