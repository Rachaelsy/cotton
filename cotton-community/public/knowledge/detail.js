...
async function explainQuiz(index, button) {
  ...
  try {
    const response = await fetch('/api/community-ai/chat', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ message, displayMessage:`讲解错题：${question.question}`, history:[] }) })
...
$('loginForm').addEventListener('submit', async event => {
  ...
  try {
    const response = await fetch('/api/community-auth/login', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ phone:$('loginPhone').value.trim(), password:$('loginPassword').value }) })
...
async function askAi() {
  ...
  try {
    const response = await fetch('/api/community-ai/chat', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ message:`我正在学习《${state.content.title}》。课程内容如下：${context}\n\n我的问题：${question}`, displayMessage:question, history:state.aiHistory }) })
