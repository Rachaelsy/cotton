(() => {
  ...
  function hidePanels() { ['listPanel', 'editorPanel', 'productPanel', 'expertPanel', 'securityPanel', 'farmerPanel', 'plotPanel'].forEach(id => $(id).classList.add('hidden')) }
  ...
  async function save(event) {
    ...
    try { await api(`/admin${id ? `/${id}` : ''}`, { method: id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); closeModal(); await load(); runtime.notify(body.status === 'published' ? '加工厂已同步发布到小程序和网页端' : '加工厂已保存', 'success') } catch (error) { $('factoryMessage').textContent = error.message }
  }
