  ...
  function create(context) {
    ...
    async function renderExperts() {
      ...
      try {
        ...
        main.innerHTML = `${moduleHero('EXPERT STUDIO', '专家讲堂', '精选问答、在线专家和视频内容均由公共服务管理员统一发布。', 'experts')}<section class="module-page"><div class="shell"><div class="expert-web-section"><div class="module-section-title"><span>问</span><div><h2>精选问答</h2><p>围绕棉花生产期间常见问题的文字解答</p></div></div><div class="module-list">${contents.filter(item => item.type === 'qa').map(expertContentRow).join('') || empty('暂无精选问答', '管理员发布问答后会显示在这里。')}</div></div><div class="expert-web-section"><div class="module-section-title"><span>专</span><div><h2>在线专家</h2><p>专家资料与小程序同步</p></div></div><div class="web-expert-grid">${experts.map(item => `<article><div class="web-expert-avatar">${item.avatarUrl ? `<img src="${escapeHtml(safeUrl(item.avatarUrl))}" alt="">` : escapeHtml(item.avatar || '专')}</div><div><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml([item.title, item.org].filter(Boolean).join(' · '))}</p><div>${(item.tags || []).map(tag => `<span>${escapeHtml(tag)}</span>`).join('')}</div></div></article>`).join('') || empty('暂无在线专家', '管理员维护专家资料后会显示在这里。')}</div></div><div class="expert-web-section"><div class="module-section-title"><span>视</span><div><h2>精选视频</h2><p>专家发布的棉花种植教学视频</p></div></div><div class="module-card-grid">${contents.filter(item => item.type === 'video').map(expertVideoCard).join('') || empty('暂无精选视频', '管理员发布视频后会显示在这里。')}</div></div></div></section>`
      } catch (error) { errorPanel(error, '/') }
    }
    ...
    function expertVideoCard(item) {
      return `<article class="module-card">${imageBlock(item.coverUrl, item.title)}<div><span class="module-badge">视频 · ${escapeHtml(item.duration || '时长待定')}</span><h2><a href="${publicLink(`/experts/${item.id}`)}">${escapeHtml(item.title)}</a></h2><p>${escapeHtml(item.expertName || item.teacher || '平台专家')}</p></div></article>`
    }
    ...
    async function renderExpertDetail(id) {
      ...
      try {
        ...
        main.innerHTML = `<article class="database-article"><header><div class="shell"><a class="article-back" href="${publicLink('/experts')}">← 返回专家讲堂</a><span class="article-channel">精选视频 · ${escapeHtml(item.categoryName)}</span><h1>${escapeHtml(item.title)}</h1><div class="database-article-meta"><span>${escapeHtml(item.expertName || item.teacher || '平台专家')}</span><span>${escapeHtml(item.org || '')}</span></div></div></header><div class="shell database-article-layout"><div class="database-article-body">${item.videoUrl ? `<video class="expert-web-video" controls preload="metadata" poster="${escapeHtml(safeUrl(item.coverUrl))}"><source src="${escapeHtml(safeUrl(item.videoUrl))}"></video>` : ''}${markdownToHtml(item.content || item.intro)}</div><aside class="database-article-aside"><strong>讲堂信息</strong><dl><div><dt>类型</dt><dd>视频</dd></div><div><dt>分类</dt><dd>${escapeHtml(item.categoryName)}</dd></div><div><dt>专家</dt><dd>${escapeHtml(item.expertName || item.teacher || '平台专家')}</dd></div></dl></aside></div></article>`
