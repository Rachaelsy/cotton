...
公益平台管理员可通过统一后台令牌使用农户列表、新增、编辑、启用和禁用功能。授权仅作用于农户管理接口；商户、订单、商品及其他核心管理接口仍只允许智能体管理员访问。

公益小程序的病虫害识别结果保存在共享后端的 `pest_recognition_records` 表中，并以登录农户 `user_id` 隔离。`GET /api/ai/photo/history` 只返回当前账号的最近记录及实际总次数，避免同一设备切换账号时共用本地历史。

公益小程序的地块气象页面使用共享天气 API：左上角下拉列表可选择当前位置或当前账号的地块，并按对应坐标重新加载天气；无地块时可按当前位置获取区域预报，定位不可用时降级为喀什参考坐标。页面展示未来 24 小时、7 日趋势、预警与农事建议。
...
当前产品边界：微信小程序仅提供农户功能；商户、农机手、专家和管理员统一从网页端进入各自后台。已有网页商户或农机手账号仍可使用同一手机号和原密码在小程序补充农户资料，网页身份保持不变，小程序按农户身份使用。农户登录后直接进入首页，实名认证为可选服务，不影响普通功能。首页暂未开放的棉花交易、农业贷款和农业保险入口继续保留，点击时统一提示“尚在开发中”。
...
- 两个项目必须使用相同的 `JWT_SECRET`。由于浏览器存储按域名隔离，社区拥有独立的用户和管理员登录页，但账号、密码和管理员权限来自共享数据库。
- 课程、评论、论坛、AI 问答、课程上传、对象存储建议和社区部署方式统一记录在 `cotton-community/README.md`。
...
- 未登录用户进入农资结算时，小程序通过 `wx.login` 静默建立微信游客身份，不要求授权手机号或注册平台账号；收货姓名、电话和地址仍由用户填写。
- 微信游客拥有独立 JWT 和订单归属，支持创建农资订单、微信支付、取消待付款订单、查看当前设备订单、物流查询和确认收货。游客不能使用账户优惠券，也不能绕过农机租赁的正式农户登录要求。
- 游客后续使用手机号或微信一键登录/注册时，当前游客身份下的农资订单会自动归并到正式农户账户。评价和退款售后需要先完成这一步，以保证长期订单归属和售后身份可信。
- 部署新版本后必须先在 `server` 目录运行 `npm run migrate:guest-checkout`；该迁移创建 `wechat_guests` 并为 `orders` 增加可空的正式用户归属和游客归属。

### 农机租赁与支付

- 农户选择机具、作业地址、日期和计费数量后创建预约；后端会重新校验农户身份、机具/农机手状态、服务范围、手机号、按亩或按天计价规则，并在事务中阻止同一机具日期重叠预约。
- 新预约默认保留 30 分钟付款时间，超时后由请求入口和定时任务共同关单。农机手只能接收已支付定金或全款的订单，未付款订单不会计入待接单统计。
- 支持定金、尾款和一次性全款三种付款阶段，每一阶段独立保存微信商户单号、交易号、实付金额、分账与退款记录；农户取消或农机手拒单时会按实际已付阶段逐笔退款。
- 农户端支持继续付款、订单跟踪、作业位置状态和完工评价；农机手网页端支持订单流转、作业定位、财务汇总、评价查看与回复。
- 真实支付统一使用微信支付服务商 Partner JSAPI。支付回调 `attach` 使用不超过 128 字节的紧凑订单标识，同时兼容旧格式，避免农机和农资订单因附加数据超限而预下单失败。
- 农机手填写的 `sub_mchid` 必须是当前 `WECHAT_PAY_SP_MCH_ID` 名下真实完成进件的特约商户号。手动录入只保存号码并显示“待微信校验”，不会在微信侧建立受理关系；微信返回 `NO_AUTH: 受理关系不存在` 时，需要在当前服务商名下重新完成特约商户进件或授权。
...
测试覆盖农户权限边界、可选实名认证、天气缓存与重试、AI 和上传安全、支付超时保护、意见反馈、在线客服实时消息、营销计价、农机预约冲突/状态流转、分阶段支付退款与主要订单流程。
...
```bash
cd server
npm run migrate:guest-checkout
```

公益课程和论坛结构由独立的 `cotton-community` 迁移：
...
积分规则为：完成入门、进阶、高阶课程分别奖励 20、30、40 分；同一课程仅奖励一次，
每日课程奖励上限 100 分。农资结算时 `100` 积分抵 `1` 元，单笔至少使用 100 分、
最多 2000 分，并受订单金额 10%、平台服务费和账户余额共同限制。积分优惠由平台承担，
商户结算仍按积分抵扣前的客户应付金额计算。待支付订单锁定积分，取消或超时释放，
支付后核销，全额退款后返还；游客免登录订单不参与积分。
...
| POST | `/api/auth/wechat-guest` | 小程序游客 | 用 `wx.login` 临时 code 建立免手机号微信游客身份 |
| POST | `/api/orders` | 农户/微信游客 | 下单（正式账户用账号信息，游客填姓名+手机+地址）|
| GET  | `/api/orders/my` | 农户/微信游客 | 查看当前身份的农资订单列表（支持 status/status2 双状态过滤）|
...
| PATCH | `/api/orders/:id/cancel` | 农户/微信游客 | 取消本人待付款订单（释放库存） |
| PATCH | `/api/orders/:id/confirm` | 农户/微信游客 | 确认本人订单收货（状态改为 completed，资金冻结 7 天） |
...
| GET/PATCH | `/api/operator/reviews` `/api/operator/reviews/:id/reply` | 机主 | 查看农户评价 / 回复评价 |
| GET   | `/api/machines?lat=&lng=&category=&sort=` | 公开 | 农机列表（真实距离排序）|
| GET   | `/api/machines/:id` | 公开 | 农机详情（含评价）|
| POST  | `/api/machine-orders` | 农户 | 提交预约（校验服务范围、计费单位、日期冲突和收款能力）|
...
machine_orders       → id, order_no, machine_id, operator_id, farmer_id, machine_name/icon,
                        plot_id, plot_name, work_address(作业地址), work_date, work_end_date,
                        work_area(计费数量), unit_price, billing_unit(亩/天), total_price,
                        deposit(定金20%), pay_mode, pay_status, pay_expires_at,
                        deposit/balance 分阶段实付金额、微信交易号与退款状态,
