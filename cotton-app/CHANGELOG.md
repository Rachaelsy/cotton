...
- **统一登录页入口精简**：左侧不再展示“商业平台”跳转卡片，仅保留公共服务平台入口；同步调整说明文案和单卡宽度。本次只隐藏公开入口，不删除既有业务账号、接口或数据。

## [未发布] — 2026-08-19

### 新增

- **地块生育期字段**：`plots` 表增加 `growth_stage`，限定为播种出苗期、苗期、蕾期、花铃期和吐絮收获期；字段留空时按播种日期推算，后台统计与小程序展示共用同一解析器。
- **地块详情解析结果**：`GET /api/plots/:id` 新增 `resolved_growth_stage` 与 `growth_stage_source`，区分人工确认和自动推算。

### 调整

- **正式域名更新**：生产 API、公开地址、微信网页授权回调、支付与退款回调统一切换为 `https://xjsmartcotton.cn`，并同步配置模板与自动化测试。

## [未发布] — 2026-08-18

### 新增

- **农户与地块综合接口**：新增 `GET /api/admin/farmer-landscape`，供授权管理员读取脱敏农户信息、地块种植与坐标、生产记录、病虫害识别、已有气象风险及县域/品种/生育期汇总；不返回证件、交易和支付数据。
...
- **公益管理员跳转保护**：核心后台页面在渲染菜单前识别公益管理员令牌；即使旧登录页或浏览器缓存误跳到 `/admin/dashboard.html`，也会立即转入只显示公益模块的独立后台。

## [2.1.0] — 2026-06-02

### 新增

- **管理员后台 — 售后管理面板**（`server/public/admin/dashboard.html`）：
  - 侧边栏新增「🔄 售后管理」入口
  - 展示全平台所有商家的售后申请，支持按状态筛选（全部 / 待处理 / 已同意 / 已拒绝）
  - 表格显示：售后 ID、订单号、申请农户、所属商户、售后类型、原因、状态、申请时间
  - 「详情」按钮弹出详情弹窗，展示申请描述和买家上传的凭证图片
  - 后端 API：`GET /api/admin/aftersales`，支持 `?status=` 查询参数

- **管理员后台 — 财务管理面板**（`server/public/admin/dashboard.html`）：
  - 侧边栏新增「💰 财务管理」入口
  - 顶部统计卡：平台总销售额、平台总佣金（按各商户实际佣金率计算）、待审批提现笔数、冻结资金总额
  - **商户财务汇总表**：列出每家已批准商户的完成销售额、平台佣金金额、可提现余额、冻结中资金、累计已提现金额、待审批提现笔数
  - **提现申请记录表**：展示全平台提现申请，支持按状态筛选（待审批 / 已到账 / 已拒绝）；待审批记录显示「批准」和「拒绝」操作按钮，拒绝时弹出原因输入框
  - 后端 API：
    - `GET /api/admin/finance` — 各商户财务汇总（含子查询聚合各状态资金）
    - `GET /api/admin/withdrawals` — 全平台提现申请列表，支持状态筛选
    - `PATCH /api/admin/withdrawals/:id/handle` — 审批提现（`action: approve/reject`）
