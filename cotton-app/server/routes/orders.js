...
// ─────────────────────────────────────────────
// POST /api/orders — 提交订单（正式农户或已建立微信身份的游客）
// ─────────────────────────────────────────────
router.post('/', principalAuth, async (req, res) => {
  ...
  if (selectedCouponId && !req.user) return fail(res, '请登录后使用优惠券', 403)
  ...
  try {
    ...
    // 正式农户使用账号资料；微信游客使用本次填写的收货资料。
    const userId = req.principal.type === 'user' ? req.principal.id : null
    const guestId = req.principal.type === 'guest' ? req.principal.id : null
    ...
    return ok(res, {
      ...
      deliveryDistance,
      guestOrder: req.principal.type === 'guest'
    }, '订单提交成功，请在30分钟内完成支付')
  } catch (e) {
    await conn.rollback()
    conn.release()
...
// ─────────────────────────────────────────────
// PATCH /api/orders/:id/cancel — 主动取消
// ─────────────────────────────────────────────
router.patch('/:id/cancel', principalAuth, async (req, res) => {
  const { id } = req.params
  try {
    const owner = ownerCondition(req.principal)
    const [rows] = await db.query(
      `SELECT id FROM orders WHERE id=? AND ${owner.sql} AND status='pending_payment'`,
      [id, ...owner.params]
    )
...
// 内部：取消订单并释放库存
// ─────────────────────────────────────────────
// GET /api/orders/my — 正式农户或当前微信游客查看自己的农资订单
// ─────────────────────────────────────────────
router.get('/my', principalAuth, async (req, res) => {
  try {
    if (req.principal.type === 'user') {
      await refunds.syncPendingSupplyRefunds({ userId: req.principal.id }).catch(error => {
    ...
    let sql = `
      SELECT o.id, o.order_no, o.original_subtotal, o.promotion_discount,
             ...
             (o.guest_id IS NOT NULL) AS is_guest_order,
      ...
      WHERE ${owner.sql} AND o.farmer_deleted = 0
    `
...
// ─────────────────────────────────────────────
// DELETE /api/orders/:id — 买家删除（隐藏）订单（仅已完成/已取消/售后完成）
// ─────────────────────────────────────────────
router.delete('/:id', principalAuth, async (req, res) => {
  ...
  try {
    ...
    const [[o]] = await db.query(
      `SELECT status FROM orders WHERE id=? AND ${owner.sql}`,
      [id, ...owner.params]
    )
...
// ─────────────────────────────────────────────
// PATCH /api/orders/:id/confirm — 正式农户或微信游客确认收货
// ─────────────────────────────────────────────
router.patch('/:id/confirm', principalAuth, async (req, res) => {
  const { id } = req.params
  try {
    const owner = ownerCondition(req.principal)
    const [rows] = await db.query(
      `SELECT id FROM orders WHERE id=? AND ${owner.sql} AND status=?`,
      [id, ...owner.params, 'shipped']
    )
