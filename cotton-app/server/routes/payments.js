...
async function loadSupplyOrder(orderId, principal, statuses = ['pending_payment']) {
  ...
  const [[row]] = await db.query(
    `SELECT o.id,o.order_no,o.total,o.original_subtotal,o.commission_base,o.status,o.pay_expires_at,
      ...
      WHERE o.id=? AND ${owner.sql} AND o.status IN (${placeholders}) GROUP BY o.id`,
    [orderId, ...owner.params, ...statuses]
  )
...
async function loadAnySupplyOrder(orderId, principal) {
  return loadSupplyOrder(orderId, principal, ['pending_payment', 'pending_ship'])
}

async function loadPayerOpenid(principal) {
  if (principal.type === 'guest') {
    const [[guest]] = await db.query(
      'SELECT openid FROM wechat_guests WHERE id=? AND is_active=1',
      [principal.id]
    )
    return guest && guest.openid || ''
  }
  const [[user]] = await db.query('SELECT openid FROM users WHERE id=?', [principal.id])
...
async function markSupplyPaid(order, principal, transaction, paymentMode = 'wechat') {
  const owner = principal ? ownerCondition(principal) : null
  const ownerSql = owner ? ` AND ${owner.sql}` : ''
  const params = [
    outTradeNo('supply', order), transaction && transaction.transaction_id || '', paymentMode, order.id
  ]
  if (owner) params.push(...owner.params)
  const [result] = await db.query(
...
router.post('/wechat/prepay', principalAuth, async (req, res) => {
  ...
  try {
    ...
    if (orderType === 'machine') {
      if (req.principal.type !== 'user') return fail(res, '农机租赁需要登录农户账户', 403)
    ...
    let payerOpenid = ''
    if (!mockPayment) {
      payerOpenid = await loadPayerOpenid(req.principal)
      if (!payerOpenid) return fail(res, '未找到当前微信身份，请重新进入结算页', 409)
    }

    const order = orderType === 'machine'
      ? await loadMachineOrder(orderId, req.user.id, paymentStage)
      : await loadSupplyOrder(orderId, req.principal)
    if (orderType === 'machine') {
...
router.post('/wechat/confirm', principalAuth, async (req, res) => {
  const orderType = req.body.orderType === 'machine' ? 'machine' : 'supply'
  const orderId = Number(req.body.orderId)
  if (!orderId) return fail(res, '缺少订单编号')
  try {
    ...
    const order = orderType === 'machine'
      ? await loadMachineOrder(orderId, req.user.id, paymentStage)
      : await loadAnySupplyOrder(orderId, req.principal)
    if (orderType === 'supply') {
      const expiryCheck = validateSupplyNotExpired(order)
      if (!expiryCheck.ok) return fail(res, expiryCheck.msg, expiryCheck.code)
    }
    ...
    if (mockPayment) {
      ...
      const changed = orderType === 'machine'
        ...
        : await markSupplyPaid(order, req.principal, transaction, 'mock')
    ...
    const changed = orderType === 'machine'
      ? await markMachinePaid(order, req.user.id, transaction)
      : await markSupplyPaid(order, req.principal, transaction, 'wechat')
    if (!changed) return fail(res, '订单状态已变化，请刷新后重试', 409)
