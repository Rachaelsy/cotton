...
> 小程序注册使用农户身份（`role: "farmer"`）。已有网页商户或农机手账号可提交同一手机号和原账号密码，在原用户下补充农户资料；网页身份不会被覆盖。
...
同手机号已有网页账号但原密码不正确时返回 `409`；已经具备农户身份时提示直接登录。

**失败示例：**
```json
{ "code": 409, "msg": "该手机号已有网页端账号，请填写原账号密码以注册农户身份" }
```
...
**小程序请求体：**
```json
{ "phone": "13800000001", "password": "abc123", "role": "farmer" }
```

`role: "farmer"` 表示请求农户会话；只要该用户已有 `farmers` 资料，即使主账号同时是商户或农机手，也会签发农户 Token。
