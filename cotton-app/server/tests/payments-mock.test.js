...
async function run() {
  ...
  try {
    ...
    calls.length = 0
    const guestToken = jwt.sign({ guestId: 77, role: 'guest', type: 'wechat_guest' }, process.env.JWT_SECRET)
    const guestPrepay = await post(baseUrl, guestToken, '/api/pay/wechat/prepay', { orderType: 'supply', orderId: 8 })
    assert.strictEqual(guestPrepay.status, 200)
    assert(calls.some(call => /o\.guest_id=\?/i.test(call.sql || '') && call.params.includes(77)),
      'guest prepay must load only the current guest order')
    const guestConfirm = await post(baseUrl, guestToken, '/api/pay/wechat/confirm', { orderType: 'supply', orderId: 8 })
    assert.strictEqual(guestConfirm.status, 200)
    assert(calls.some(call => /AND guest_id=\?/i.test(call.sql || '') && call.params.includes(77)),
      'guest payment confirmation must update only the current guest order')
