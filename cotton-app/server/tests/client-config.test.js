...
function run() {
  ...
  assert.strictEqual(prodUrl, 'https://xjsmartcotton.cn', 'production API should use the configured HTTPS host')
  assert.ok(!prodUrl.endsWith('/'), 'production API base URL should not end with a slash')
  assert.ok(!prodUrl.includes('www.xjsmartcotton.cn'), 'production API should use the configured apex domain')
