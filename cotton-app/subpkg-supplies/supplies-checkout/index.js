...
Page({
  ...
  async onPay() {
    ...
    let identity
    try {
      identity = await auth.ensureGuestSession()
    } catch (error) {
      this.setData({ submitting: false })
      wx.showModal({
        title: '暂时无法下单',
        content: error.message || '未能建立微信身份，请稍后重试',
        showCancel: false
      })
      return
    }
    ...
    for (const group of groups) {
      ...
      // Bug5: 下单失败时终止流程并提示
      try {
        const res = await auth.guestRequest('POST', '/api/orders', {
          items: orderItems,
          subtotal,
          deliveryFee,
          total,
        ...
        wx.showModal({
          ...
          content: e.message || this.data.copy.network,
          showCancel: false
        })
        return
      }
