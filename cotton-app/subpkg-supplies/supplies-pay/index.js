...
Page({
  ...
  async _loadPaymentMode() {
    try {
      const result = await auth.guestRequest('GET', '/api/pay/wechat/mode')
  ...
  async onPay() {
    ...
    for (const o of orders) {
      ...
      try {
        const prepay = await auth.guestRequest('POST', '/api/pay/wechat/prepay', {
          orderType: 'supply',
          orderId: o.orderId
        })
        ...
        const confirm = await auth.guestRequest('POST', '/api/pay/wechat/confirm', {
          orderType: 'supply',
          orderId: o.orderId
        })
  ...
  async onCancel() {
    ...
    for (const o of orders) {
      if (!o.orderId) continue
      await auth.guestRequest('PATCH', `/api/orders/${o.orderId}/cancel`, {}).catch(() => {})
    }
    if (this._timer) clearInterval(this._timer)
