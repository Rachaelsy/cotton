...
Page({
  ...
  onAddToCart() {
    const p = this.data.product
    if (!p) return
    const current = (app.globalData.cart || []).find(item => String(item.id) === String(p.id))
    const stock = Number(p.stock)
    if (Number.isFinite(stock) && (stock <= 0 || Number(current?.qty || 0) >= stock)) {
      wx.showToast({ title: this.data.copy.stockOut || '库存不足', icon: 'none' })
      return
    }
    app.addToCart(p)
