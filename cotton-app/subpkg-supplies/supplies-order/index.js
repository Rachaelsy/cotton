...
const COPY = {
  zh: {
    ...
    payMethod: '支付方式', logisticsNo: '物流单号', subtotal: '商品原价', discount: '商户优惠', pointsDiscount: '平台积分抵扣', freight: '运费', paid: '实付金额',
  ...
  ug: {
    ...
    payMethod: 'تۆلەش ئۇسۇلى', logisticsNo: 'يەتكۈزۈش نومۇرى', subtotal: 'مال ئەسلى باھاسى', discount: 'سودىگەر ئېتىبارى', pointsDiscount: 'سۇپىنىڭ نومۇر ئېتىبارى', freight: 'توشۇش ھەققى', paid: 'تۆلەنگەن پۇل',
...
Page({
  ...
  async _loadOrder() {
      ...
      try {
        ...
        if (res.code === 200 && Array.isArray(res.data)) {
          ...
          if (fresh) {
            ...
            order.originalSubtotal = fresh.original_subtotal
            order.merchantDiscount = fresh.merchant_discount
            order.pointsUsed = fresh.points_used || 0
            order.pointsDiscount = fresh.points_discount || 0
            order.subtotal = fresh.subtotal
            order.deliveryFee = fresh.delivery_fee
            order.total = fresh.total
            order.has_reviewed = fresh.has_reviewed
            order.is_guest_order = !!fresh.is_guest_order
          }
        }
      } catch (e) {
    ...
    if (saved.orderId && !order.is_guest_order && auth.isLoggedIn()) {
      try {
  ...
  async _loadLogistics(refresh) {
    ...
    try {
      ...
      const result = await auth.guestRequest('GET', path)
      if (result.code !== 200) {
  ...
  onAfterSale() {
    if (this.data.isGuestOrder || !auth.isLoggedIn()) {
      this._promptAccountForService('登录后可以申请退款或售后，当前游客订单会自动同步到账号。')
      return
    }
  ...
  onConfirmReceipt() {
    wx.showModal({
      ...
      success: async (res) => {
        ...
        if (!orderId) return
        try {
          const result = await auth.guestRequest('PATCH', `/api/orders/${orderId}/confirm`)
          if (result.code === 200) {
  ...
  onReview() {
    if (this.data.isGuestOrder || !auth.isLoggedIn()) {
      this._promptAccountForService('登录后可以评价商品，当前游客订单会自动同步到账号。')
      return
    }
  ...
  _promptAccountForService(content) {
    wx.showModal({
      title: '需要登录账户',
      content,
      confirmText: '去登录',
