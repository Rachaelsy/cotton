...
公益管理员统一从 `/admin/login.html?role=admin` 登录。后台左侧包含“政策资讯”“农户管理”和“账号安全”；退出后返回统一登录页，旧的公益运营独立登录页已删除。
...
- 生产服务：首页提供农机、农资、加工三个独立模块。农机页按作业类型和机型介绍组织棉田机械知识；农资页介绍种子、肥料、农药、地膜的常见类型、选购核验和安全注意事项；加工页承接轧花、仓储和运输信息。当前只提供公益信息和入口指引，不直接交易、收款或提供履约担保。
...
公益小程序拥有独立、可扩展的管理后台。管理员从统一入口 `/admin/login.html?role=admin` 登录，进入“政策资讯”后选择内容类型与分类，使用 Markdown 编辑、实时预览、保存草稿和发布。发布内容通过 `/api/policies` 自动进入小程序对应的政策或行业栏目。创建账号和部署步骤见 `cotton-community/README.md`。
...
│  ├─ production/                 农机、农资、加工生产服务入口
│  ├─ machinery/                  农机作业类型与机型介绍
│  ├─ supplies/                   种子、肥料、农药、地膜介绍
│  ├─ academy/                    棉知学堂、课程、考试
...
- 公益平台独有：资讯栏目、课程体系、学习进度、考试、学习积分、证书和棉花金融科普阅读状态。
